...
class QuicktestBase:
    """
    Quicktest base class. Simple backtesting class. Instantiate with a dataframe of signals and prices and then use the various stats methods such as sharpe and account curve etc. Works best with end-of-day prices and signals

    - signals: dataframe of buy/sell timeseries, usually RMS 1, one column for each market
    - prices: dataframe of prices (one column for each market)
    """
    ...
    def __init__(self,
        ...
        signals = signals.T.sort_index().T  # The (T.sort_index().T) is just ordering all of the columns in alphabetical order
        prices = prices.T.sort_index().T  # The (T.sort_index().T) is just ordering all of the columns in alphabetical order
    ...
    def price_returns_vol(self):
        ...
        return l.daily_volatility(price_returns, lookback=self.vol_lookback)
    ...
    def account_curve(self, mask=None, lag=None):
        ...
        * lag will default to self.lag if lag=None when market_returns is called
        ...
        returns = market_returns.mean(axis=1).mul(
            gearing)  # By taking the mean I am assuming that I am equally pweighting all of the market returns here
    ...
    def sharpe(self, lag=None):
        ...
        sharpe = l.sharpe(returns)
        return sharpe

    def holding_period(self):
        return l.holding_period(self.reindexed_signals())
    ...
    def long_short_stats(self):
        ...
        long_sharpe = l.sharpe(long_account_curve.diff())
        short_sharpe = l.sharpe(short_account_curve.diff())
        ...
        return pd.Series(
            {"long_sharpe": long_sharpe, "short_sharpe": short_sharpe, "percentage_long": percentage_long}).to_frame(
            "Stats").T

    def stats(self):
        stats = pd.Series({"sharpe": round(self.sharpe(), 2), "holding_period": self.holding_period()}).to_frame(
            "Stats").T
        return stats
    ...
    def tilt_timing_stats(self):
        ...
        tilt_backtest = QuicktestBase(tilt_signals, self.prices, default_lag=self.default_lag,
                                      uncorrelated_markets=self.uncorrelated_markets)
        timing_backtest = QuicktestBase(timing_signals, self.prices, default_lag=self.default_lag,
                                        uncorrelated_markets=self.uncorrelated_markets)

        sharpes = pd.Series(
            {"backtest": self.sharpe(), "tilt": tilt_backtest.sharpe(), "timing": timing_backtest.sharpe()}).to_frame(
            "sharpes").T
        account_curves = pd.concat(
            [self.account_curve(), tilt_backtest.account_curve(), timing_backtest.account_curve()],
            keys=["backtest", "tilt", "timing"], axis=1).diff().dropna(how="any").cumsum()
    ...
    def market_stats(self):
        ...
        market_sharpes = l.sharpe(self.market_returns()).sort_values(ascending=False)
        market_correlations = market_account_curves.diff(5).corr()
        if len(market_correlations) > 1:
            market_correlations = l.cluster_matrix(market_correlations)
        sharpes = market_sharpes.to_frame("Sharpes").T
        return market_account_curves, market_correlations, sharpes


class Quicktest(QuicktestBase):
    """
    Extension of base class to include:
    - a comparison with timeseries price momentum (Moskowitz) plus a report method
    - a .report() method then prints out a backtest report when called in Jupyter notebook

    This is the class you should use for backtesting
    """
    ...
    def momentum_stats(self):
        ...
        momentum_backtest = QuicktestBase(momentum_signals, self.prices)
        momentum_account_curve = momentum_backtest.account_curve()

        backtest_account_curve = self.account_curve()
        combined_account_curves = pd.concat([backtest_account_curve, momentum_account_curve],
                                            keys=["Backtest", "Momentum"], axis=1)
        combined_returns = combined_account_curves.diff().dropna(how="any")
        combined_account_curves = combined_returns.cumsum()

        sharpes = l.sharpe(combined_returns)
        backtest_sharpe = sharpes.Backtest
        momentum_sharpe = sharpes.Momentum
        correlation = combined_account_curves.diff(5).corr().iloc[0, 1]
        appraisal = l.appraisal(backtest_sharpe, momentum_sharpe, correlation)
        stats = pd.DataFrame(
            dict(backtest_sharpe=backtest_sharpe, momentum_sharpe=momentum_sharpe, correlation=correlation,
                 appraisal=appraisal), index=["Momentum Stats"])

        return combined_account_curves, stats

    def long_only_stats(self):
        long_only_signals = copy(self.signals)
        long_only_signals = long_only_signals.map(lambda x: 1.0)
        long_only_backtest = QuicktestBase(long_only_signals, self.prices)
        long_only_account_curve = long_only_backtest.account_curve()

        backtest_account_curve = self.account_curve()
        combined_account_curves = pd.concat([backtest_account_curve, long_only_account_curve],
                                            keys=["Backtest", "Long Only"], axis=1)
        combined_returns = combined_account_curves.diff().dropna(how="any")
        combined_account_curves = combined_returns.cumsum()
        ...
        correlation = combined_account_curves.diff(5).corr().iloc[0, 1]
        appraisal = l.appraisal(backtest_sharpe, long_only_sharpe, correlation)
        stats = pd.DataFrame(
            dict(backtest_sharpe=backtest_sharpe, long_only_sharpe=long_only_sharpe, correlation=correlation,
                 appraisal=appraisal), index=["Long Only Stats"])
    ...
    ###
    def report(self):
        l.display_markdown("## Stats")
        stats = self.stats()
        display(HTML(stats.to_html()))

        l.display_markdown("## Account Curve")
        self.account_curve().plot(title="Account Curve")
        plt.show()

        l.display_markdown("## Signals")
        ...
        l.display_markdown("## Market Stats")
        ...
        l.display_markdown("## Market Correlations")
        ...
        try:
            l.display_markdown("## Tilt vs Timing")
            self.plot_tilt_timing()
        except Exception as e:
            l.display_markdown("## Tilt vs Timing")
            print(f"Error displaying Tilt vs Timing, error: {e}")

        l.display_markdown("## Long vs short")
        ...
        l.display_markdown("## Momentum Comparison")
...
class Comparison:
    """
    Class for comparing multiple backtests
    """
    ...
    @classmethod
    def compare(cls, backtests, plot_lead_lag=True, correlation_lookback=5):
        """
        Class method for creating and calling a Comparison instance.
        """
        comparison = cls(backtests, plot_lead_lag, correlation_lookback)
        comparison()

    def account_curves(self):
        account_curves = pd.DataFrame(
            {backtest_name: backtest.account_curve() for backtest_name, backtest in self.backtests.items()})
    ...
    def correlations(self):
        ...
        correlations = l.cluster_matrix(correlations)
        correlations = round(correlations, 2)
        return correlations

    def stats(self):
        stats = pd.DataFrame(
            {backtest_name: backtest.stats().squeeze() for backtest_name, backtest in self.backtests.items()})
        return stats

    def momentum_stats(self):
        stats = pd.DataFrame({backtest_name: backtest.momentum_stats()[1].squeeze() for backtest_name, backtest in
                              self.backtests.items()}).T
        return stats

    def sharpes(self):
        sharpes = l.sharpe(self.returns())
    ...
    def report(self):
        if len(self.backtests) == 1:
            ...
            l.display_markdown("## Correlations")
            correlations = self.correlations()
            l.heatmap(correlations.round(2))
            plt.show()
            ...
            if self.plot_lead_lag:
                l.display_markdown("## Lead-Lag Plots")
                for backtest_name, backtest in self.backtests.items():
                    l.ssheader(backtest_name)
